...
### Sigmoid Function
Converts model output to a probability (0 to 1):

$$
g(z) = \frac{1}{1 + e^{-z}}
$$

### Cost Function (Log Loss + L2)
Measures the model's error, which we try to minimize:

$$
J(\mathbf{w},b) = \frac{1}{m} \sum_{i=1}^{m} \Big[ -y^{(i)} \log(g(z^{(i)})) - (1-y^{(i)}) \log(1-g(z^{(i)})) \Big] + \frac{\lambda}{2m} \sum_{j=1}^{n} w_j^2
$$

### Gradient
The "direction" of the steepest cost. Used by gradient descent to update $\mathbf{w}$ and $b$:
